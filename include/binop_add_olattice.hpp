...
struct _plus {
	...
	template<typename T,typename V, size_t N, size_t Id1, size_t Id2>
	KOKKOS_INLINE_FUNCTION
	auto operator()(const PMatrix<T,V,N,Id1,Id2>& l,
		 	        const PMatrix<T,V,N,Id1,Id2>& r) const {
	...
	// PVector: view + view
	template<typename T1,typename T2, typename V, size_t N, size_t Id>
	KOKKOS_INLINE_FUNCTION
	auto operator()(const PVector<T1,V,N,Id>& l, const PVector<T2,V,N,Id>& r) const {
	...
	template<typename T1, typename T2, typename V, size_t N, size_t Id>
	KOKKOS_INLINE_FUNCTION
	auto operator()(const PVectorLocal<T1,N>& l, const PVector<T2,V,N,Id>& r) const {
		return add_vector<decltype(l),decltype(r)>(l,r);
	}
	...
	template<typename T1,typename T2, typename V, size_t N, size_t Id>
	KOKKOS_INLINE_FUNCTION
	auto operator()(const PVector<T1,V,N,Id>& l, const PVectorLocal<T2,N>& r) const {
		return add_vector<decltype(l),decltype(r)>(l,r);
	}
	...
	// Complex: view + view
	template<typename T,typename V, size_t Id, size_t NumDims>
	KOKKOS_INLINE_FUNCTION
	auto operator()(const RComplex<T,V,Id,NumDims>& l,
			        const RComplex<T,V,Id,NumDims>& r) const {
	...
	// RScalar: view + view
	template<typename T, typename V, size_t Ndims>
	KOKKOS_INLINE_FUNCTION
	auto operator()( const RScalar<T,V,Ndims>& l, const RScalar<T,V,Ndims>& r) const {
	...
	template<typename T, typename V, size_t Ndims>
	KOKKOS_INLINE_FUNCTION
	auto operator()( const RScalarLocal<T>& l, const RScalar<T,V,Ndims>& r) const {
		return add_scalar<decltype(l),decltype(r)>(l,r);
	}
	...
	template<typename T, typename V, size_t Ndims>
	KOKKOS_INLINE_FUNCTION
	auto operator()( const RScalar<T,V,Ndims>& l, const RScalarLocal<T>& r) const {
		return add_scalar<decltype(l),decltype(r)>(l,r);
	}
...
template<typename T, typename V, size_t IdxPos, typename Expr>
auto operator+( const OLattice<T,V,IdxPos>& l, const Expr& r ) {
	return OLatBinOp< _plus,
					 OLeaf<OLattice<T,V,IdxPos>>,
					 Expr>(
							OLeaf<OLattice<T,V,IdxPos>>(l), r.self());
}
#endif
// This OLat + an OLatExpr and creates a BinOp
template<typename T, typename V, size_t IdxPos, typename Expr>
auto operator+( const Expr& l, const OLattice<T,V,IdxPos>& r) {
	return OLatBinOp<  _plus,
					Expr,
					OLeaf<OLattice<T,V,IdxPos>> >(
							l.self(), OLeaf<OLattice<T,V,IdxPos>>(r));
}


// This OLat + OLat and creates a BinOp
template<typename T, typename V, size_t IdxPos>
auto operator+( const OLattice<T,V,IdxPos> l, const OLattice<T,V,IdxPos>& r) {
	return OLatBinOp< _plus,
					OLeaf<OLattice<T,V,IdxPos>>,
					OLeaf<OLattice<T,V,IdxPos>> >(
							OLeaf<OLattice<T,V,IdxPos>>(l), OLeaf<OLattice<T,V,IdxPos>>(r));
}
